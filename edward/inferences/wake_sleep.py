...
def wake_sleep(model, variational, align_latent, align_data,
               scale=None, n_samples=1, phase_q='sleep',
  ...
  for s in range(n_samples):
    ...
    if phase_q == 'sleep':
      with Trace() as model_trace:
        call_function_up_to_args(model, *args, **kwargs)
      intercept = make_sleep_intercept(
      ...
      for name, node in six.iteritems(posterior_trace):
        rv = node.value
        # TODO prior name. for now, build function assuming finite and
        # also pass into align_latent for intercept
        scale_factor = scale(name)
        q_log_prob[s] += tf.reduce_sum(
...
def make_sleep_intercept(trace, align_data, align_latent, args, kwargs):
